...
if sys.version_info < (3, 8):
    subprocess.run_pip("install importlib-metadata", "importlib-metadata for depthmap script")
    import importlib_metadata
else:
    import importlib.metadata as importlib_metadata
if not subprocess.is_installed('packaging'):
    subprocess.run_pip("install packaging", "packaging requirement for depthmap script")
from packaging.version import Version

def ensure(module_name, min_version=None):
    if subprocess.is_installed(module_name):
...
if not subprocess.is_installed("moviepy"):
    subprocess.run_pip('install "moviepy==1.0.2"', "moviepy requirement for depthmap script")
...
if not subprocess.is_installed("networkx"):
    subprocess.run_pip('install install "networkx==2.5"', "networkx requirement for depthmap script")
...
def try_install_from_wheel(pkg_name: str, wheel_url: str):
    if get_installed_version(pkg_name) is not None:
        return
    try:
        subprocess.run_pip(f"install {wheel_url}", f" {pkg_name} requirement for depthmap script")
